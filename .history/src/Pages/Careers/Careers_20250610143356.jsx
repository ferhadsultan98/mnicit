    ...
    <section className="careersSection" id='careers'>
      <div className="careersContainer">
        ...
        <div className="jobFilters">
          <h3 className="jobsTitle">Açıq Vəzifələr</h3>
          ...
          <div className="filtersContainer">
            ...
            <div className="departmentTabs">
              {departments.map(dept => (
                <button
                  key={dept.id}
                  className={`departmentTab ${selectedDepartment === dept.id ? 'active' : ''}`}
                  onClick={() => setSelectedDepartment(dept.id)}
                >
                  {dept.name}
                  <span className="tabCount">{dept.count}</span>
                </button>
              ))}
            </div>
        ...
        <div className="jobListings">
          {filteredJobs.length > 0 ? (
              ...
              <div key={job.id} className="jobCard">
                <div className="jobCardHeader">
                  ...
                  <div className="jobLevel">
                    <span className={`levelBadge ${job.level.toLowerCase()}`}>
                      {job.level}
                    </span>
                  </div>
                </div>
                ...
                <div className="jobCardFooter">
                  <div className="jobStats">
                    ...
                    <span className="jobApplicants">
                      <FaUsers /> {job.applicants} müraciət
                    </span>
                  </div>
                  <button className="applyButton">
                    Müraciət et <FaChevronRight />
                  </button>
                </div>
              </div>
            ))
            ...
            <div className="noJobs">
              <h4>Axtardığınız meyarlara uyğun vəzifə tapılmadı.</h4>
              <p>Filtrləri dəyişdirməyə cəhd edin və ya sonra yenidən yoxlayın.</p>
            </div>
          )}
        </div>

        <div className="careersFooter">
          <h3>Axtardığınız Vəzifəni Tapmadınız?</h3>
          <p>
            CV-nizi bizimlə paylaşın, uyğun vəzifə açıldığında sizinlə əlaqə saxlayaq.
          </p>
          <div className="ctaButtons">
            <button className="cvButton">CV Göndər</button>
