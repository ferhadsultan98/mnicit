    ...
    <header className={`header ${isScrolled ? 'scrolled' : ''}`}>
      <div className="header-container">
        <div className="logo" onClick={() => scrollToSection('top')}>
          <img src={isScrolled ? MNLogoScrolled : MNLogoDefault} alt="MN ICIT Loqosu" />
        </div>
        ...
        <nav className={`nav-menu ${isMenuOpen ? 'active' : ''}`}>
          ...
          <div className="language-switcher" ref={dropdownRef}>
            <button className="lang-toggle" onClick={toggleDropdown}>
              {i18n.language.toUpperCase()} <span className="dropdown-icon">{isDropdownOpen ? <IoIosArrowUp fontSize={24}/> : '▼'}</span>
              {isDropdownOpen ? <IoIosArrowUp fontSize={24}/> : <IoIosArrowDown fontSize={24}/>}
            </button>
